...
class ThreadPool
{
    public:
        /*!
         * \brief Split a range [from,to) into n equal non-overlapping intervals
         * [from,b), [b,c), .... [d,to) where [d,to) is the n-th slice.
         * This function is usefull to compute the boundary indices in between
         * which worker threads should work on a common data structure.
         ...
         */
        static std::vector<std::pair<size_t,size_t>> split_range(size_t from,
                                                                 size_t to,
                                                                 size_t n) ;
